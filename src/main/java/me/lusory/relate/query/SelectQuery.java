...
import me.lusory.relate.mapping.LcEntityReader;
...
public class SelectQuery<T> {
    ...
    /**
     * Create a join, using the link entityName.propertyName, and using the given alias as the
     * joined entity name.
     */
    ...
    /**
     * Set the given criteria in the where clause. If criteria already exist, a AND is created
    ...
    public SelectQuery<T> orderBy(String entityName, String propertyName, boolean ascending) {
        this.orderBy.add(Tuples.of(entityName, propertyName, ascending));
        return this;
    }
    ...
    /**
     * Execute a COUNT request, using the joins and where clause of this select query. Any join
     * which are not useful (not used for conditions) are automatically ignored.
     */
    ...
    void setJoinsTargetType(LcMappingR2dbcConverter mapper) {
        for (int i = 0; i < joins.size(); ++i) {
            ...
            LcEntityTypeInfo.ForeignTableInfo ft = sourceInfo.getForeignTableWithFieldForProperty(join.propertyName);
            if (ft != null) {
                if (ft.isCollection()) {
            ...
            if (joinInfo != null) {
                TableReference newJoin =
                        new TableReference(
                                join.source,
                                joinInfo.getJoinForeignTable().getField().getName(),
                                ModelUtils.getCollectionType(
                                        joinInfo.getJoinForeignTable().getField()),
                                join.source.alias + "__join__" + join.alias);
                join.source = newJoin;
            ...
            throw new ModelAccessException(
                    "Cannot join on property "
                            + join.source.targetType.getSimpleName()
                            + '#'
                            + join.propertyName);
        }
    }
    ...
    public String toString() {
        ...
        s.append("SELECT FROM ")
                .append(from.targetType.getSimpleName())
                .append(" AS ")
                .append(from.alias);
    ...
    static class TableReference {
        ...
        private TableReference(
                TableReference source, String propertyName, Class<?> targetType, String alias) {
            this.source = source;
