...
import me.lusory.relate.model.EntityCache;
import me.lusory.relate.model.ModelUtils;
import me.lusory.relate.model.PropertiesSource;
...
public class LcEntityReader {
    ...
    public <T> T read(Class<T> type, PropertiesSource source) {
        ...
        if (conversions.hasCustomReadTarget(sourceType, rawType)
                && conversionService.canConvert(sourceType, rawType)) {
        ...
        if (entityType.requiresPropertyPopulation()) {
            ConvertingPropertyAccessor<T> propertyAccessor =
                    new ConvertingPropertyAccessor<>(
                            entityType.getPropertyAccessor(result), conversionService);
    ...
    protected Object readProperty(
            RelationalPersistentProperty property, PropertiesSource source, Object instance) {
    ...
    /**
     * Checks whether we have a custom conversion for the given simple object. Converts the given
     * value if so, applies {@link Enum} handling or returns the value as is.
     *
     * @param value
    ...
    protected <T> T readEntityProperty(
            RelationalPersistentProperty property, Object parentInstance, PropertiesSource source) {
        @SuppressWarnings("unchecked")
        ...
        throw new MappingException(
                "Sub-entity without @ForeignKey is not supported: " + property.getName());
    }

    protected <T> T readForeignKeyEntity(
    ...
    protected <T> T getOrCreateInstance(
            RelationalPersistentEntity<T> entityType, PropertiesSource source) {
        Object id;
    ...
    protected <T> T getOrCreateInstance(
        ...
        PropertiesSourceParameterValueProvider parameterValueProvider =
                new PropertiesSourceParameterValueProvider(entityType, source, conversionService);
        T instance =
                client.getMapper()
    ...
    public static class PropertiesSourceParameterValueProvider
        ...
        public PropertiesSourceParameterValueProvider(
