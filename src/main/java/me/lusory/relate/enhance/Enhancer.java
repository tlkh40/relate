package me.lusory.relate.enhance;
...
public final class Enhancer {
    ...
    private final Map<CtClass, Map<String, JoinTableInfo>> joinTableFields = new HashMap<>();
    ...
    private static void enhanceSetter(CtField field, CtMethod setter)
            throws CannotCompileException {
        setter.insertBefore(
                "if ($0._lcState != null) { $0._lcState.fieldSet(\""
                        + field.getName()
                        + "\", $1); }");
    }

    private static void createJoinTableField(
            CtClass joinClass,
            String fieldName,
            CtClass targetType,
            String columnName,
        ...
        if (columnName.length() > 0) {
            ...
            annot.addMemberValue(
                    DEFAULT_VALUE_ANNOTATION_ATTRIBUTE,
                    new StringMemberValue(columnName, constPool));
            attr.addAnnotation(annot);
        }
        field.getFieldInfo().addAttribute(attr);
    }

    private static void processJoinTableAccessors(
            CtClass cl, String propertyName, JoinTableInfo joinTable)
            throws CannotCompileException {
        String accessorSuffix =
    ...
    private static void enhanceJoinTableGetter(
            CtField field, CtMethod getter, JoinTableInfo joinTable) throws CannotCompileException {
        String body = '{' +
                "if ($0." +
                field.getName() +
                " != null) return $0." +
                ...
                "_join != null) return $0." +
                field.getName() +
                " = new net.lecousin.reactive.data.relational.model.JoinTableCollectionToTargetCollection($0, $0." +
                field.getName() +
                "_join, \"" +
                joinTable.joinClassName +
                "\", " +
                joinTable.linkNumber +
                ");" +
                "return null;" +
                '}';
        getter.setBody(body);
    ...
    private static void enhanceJoinTableSetter(
            CtField field, CtMethod setter, JoinTableInfo joinTable) throws CannotCompileException {
        String body = '{' +
                "$0." +
                field.getName() +
                "_join = new net.lecousin.reactive.data.relational.model.JoinTableCollectionFromTargetCollection($0, $0." +
                field.getName() +
                "_join, $1, \"" +
                joinTable.joinClassName +
                "\", " +
    ...
    private void enhanceClasses(Collection<String> entityClasses) throws ModelException {
        ...
        for (Map.Entry<CtClass, Map<String, JoinTableInfo>> classEntry :
                joinTableFields.entrySet()) {
            for (Map.Entry<String, JoinTableInfo> propertyEntry :
                    classEntry.getValue().entrySet()) {
                try {
                    processJoinTableAccessors(
                            classEntry.getKey(), propertyEntry.getKey(), propertyEntry.getValue());
                } catch (Exception e) {
                    throw new ModelException(
                            "Error enhancing join table accessors for "
    ...
    private List<Class<?>> loadIntoJvm(Collection<CtClass> classes) throws ModelException {
        ...
        for (CtClass cl : classes) {
            try {
                ...
                throw new ModelException(
                        "Unable to load enhanced class " + cl.getName() + " into JVM", e);
            }
        }
        return result;
    }
    ...
    private void addStateAttribute(CtClass cl) throws CannotCompileException, NotFoundException {
        CtField f =
                new CtField(
                        classPool.get("net.lecousin.reactive.data.relational.enhance.EntityState"),
                        STATE_FIELD_NAME,
                        cl);
        cl.addField(f);
    ...
    private void processJoinTables() throws ModelException {
                ...
                try {
                    ...
                    if (!(ot instanceof SignatureAttribute.ClassType)) {
                        throw new ModelException(
                                "Unexpected type "
                                        + ot
                                        + " for @JoinTable field, must be a Set with specified type: "
                                        + cl.getName()
                                        + '#'
                                        + field.getName());
                    }
                    ...
                    throw new ModelException(
                            "Error getting @JoinTable field info for "
                                    + cl.getName()
                                    + '#'
                                    + field.getName(),
                            e);
        ...
        while (!joins.isEmpty()) {
            ...
            try {
                ...
                throw new ModelException(
                        "Error generating join table entity from "
                                + t.getT1().getName()
                                + '#'
                                + t.getT2().getName(),
                        e);
    ...
    private void createJoinTable(
            Tuple4<CtClass, CtField, JoinTable, CtClass> t,
            LinkedList<Tuple4<CtClass, CtField, JoinTable, CtClass>> joins)
            throws ModelException, ReflectiveOperationException, CannotCompileException,
            NotFoundException {
        ...
        annot.addMemberValue(
                DEFAULT_VALUE_ANNOTATION_ATTRIBUTE, new StringMemberValue(tableName, constPool));
        attr.addAnnotation(annot);
        joinClassFile.addAttribute(attr);

        createJoinTableField(
                joinClass, JOIN_TABLE_ATTRIBUTE_PREFIX + "1", class1, columnName1, constPool);
        createJoinTableField(
    ...
    private void createJoinField(
            CtClass cl, CtField joinField, String joinClassName, int linkNumber)
        ...
        CtField field =
                new CtField(
                        classPool.get(Collection.class.getName()),
        ...
        annot.addMemberValue(
                "joinKey",
                new StringMemberValue(JOIN_TABLE_ATTRIBUTE_PREFIX + linkNumber, constPool));
        attr.addAnnotation(annot);
        field.getFieldInfo().addAttribute(attr);
    ...
    private void enhanceLazyMethods(CtClass cl)
        ...
        for (CtMethod method : cl.getMethods()) {
            if (method.getName().equals("entityLoaded")
                    && method.getParameterTypes().length == 0
                    && method.getReturnType().getName().equals("boolean")) {
                method.setBody("return $0._lcState != null && $0._lcState.isLoaded();");
            } else if (method.getName().equals("loadEntity")
                    && method.getParameterTypes().length == 0
                    && method.getReturnType().getName().equals(Mono.class.getName())) {
        ...
        if (!hasLoadEntity) {
            CtMethod m =
                    CtNewMethod.make(
                            "public reactor.core.publisher.Mono loadEntity() { return $0._lcState.load($0); }",
                            cl);
            cl.addMethod(m);
        }
    ...
    private void enhanceLazyGetMethods(CtClass cl)
            throws ReflectiveOperationException, CannotCompileException, NotFoundException {
        for (CtMethod method : cl.getMethods()) {
            if (method.getName().startsWith("lazyGet")) {
                ...
                if (ft != null) {
                    if (field.getType().isArray()
                            || field.getType()
                            .subtypeOf(classPool.get(Collection.class.getName()))) {
                        method.setBody(
                                "return $0._lcState.lazyGetForeignTableCollectionField($0, \""
                                        + propertyName
                                        + "\", \""
                                        + ft.joinKey()
                                        + "\");");
                        ...
                        method.setBody(
                                "return $0._lcState.lazyGetForeignTableField($0, \""
                                        + propertyName
                                        + "\", \""
                                        + ft.joinKey()
                                        + "\");");
                ...
                if (jt != null) {
                    ...
                    method.setBody(
                            "return $0._lcState.lazyGetJoinTableField($0, \""
                                    + propertyName
                                    + "\", "
                                    + info.linkNumber
                                    + ");");
                ...
                if (fk != null) {
                    ...
                    method.setBody(
                            "return $0._lcState.load($0).map($0._lcState.getFieldMapper($0, \""
                                    + propertyName
                                    + "\"));");
                }
            }
