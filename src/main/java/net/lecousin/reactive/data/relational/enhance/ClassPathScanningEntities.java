    ...
    public static Set<String> scan() throws IOException {
        ...
        Resource[] classResources =
                resourceResolver.getResources(
                        ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX + "**/*.class");
        ...
        for (Resource classResource : classResources) {
            try {
                MetadataReader metadataReader =
                        metadataReaderFactory.getMetadataReader(classResource);
    ...
    public static Class<?> searchNonEntityClass(String packageName) throws IOException {
        ...
        for (Resource classResource : classResources) {
            try {
                ...
                AnnotationMetadata annotationMetadata = metadataReader.getAnnotationMetadata();
                if (!annotationMetadata.hasAnnotation(annotationName)) {
                    return ClassPathScanningEntities.class
                            .getClassLoader()
                            .loadClass(annotationMetadata.getClassName());
                }
            } catch (
                    @SuppressWarnings("java:S1181")
                            Throwable t) {
